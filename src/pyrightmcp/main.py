...
def create_mcp_server(allowed_directories: list[str]) -> FastMCP:
    """
    Create and configure the FastMCP server for pyright.

    Args:
        allowed_directories (list[str]): List of allowed project directories.
    ...
    async def run_pyright(project_dir: str, target_dir: str, ctx: Context) -> str:
        ...
        if not any(project_path == allowed or project_path.is_relative_to(allowed) for allowed in allowed_paths):
        ...
        match result:
            ...
            case m.PyrightResult() as pyright_result:
                await ctx.info(f"Pyright completed with exit code {pyright_result.exit_code}")
...
def main():
    ...
    def serve(
        ...
        transport: str = typer.Option("stdio", help="Transport type (stdio, sse, streamable-http)"),
        ...
        mcp = create_mcp_server(allowed_directories=allowed_dirs)
